class Exam < ApplicationRecord
  ...
  has_one :lesson_member, ->(o){ where(member_id: o.member_id) }, primary_key: 'lesson_id', foreign_key: 'lesson_id'
  ...
  after_initialize if: :new_record? do
    self.state ||= 'init'
    self.lesson_id = self.lesson_paper&.lesson_id
  end
  ...
  def set_finish
    ...
    if self.lesson_paper.is_a?(ExamPaper)
  ...
  def update_score(params)
    ...
    lesson_member&.score = self.answer_mark

    if Integer(answer_mark) >= 60
      lesson_member&.state = 'passed'
    else
      lesson_member&.state = 'failed'
    end
  ...
  def set_reviewer
    if self.lesson_paper.is_a?(CertifiablePaper)
  ...
  def close_date
    if lesson_paper.is_a?(CertifiablePaper)
  ...
  def to_survey_monkey
    path = "surveys/#{lesson_paper.entity_id}/collectors"
    body = {
      type: 'weblink',
      name: member.name,
      redirect_type: 'url',
  ...
  def to_one_drive
    ...
    if ms_user
      response = ms_user.one_drive.put CGI.escape("/me/drive/root:/FolderA/#{lesson_paper.document.filename.to_s}:/content"), body: lesson_paper.document.download
  ...
  def sync_answer_detail
    ...
    _details = {}
    pages.each do |page|
      ques = SurveyMonkey.get "surveys/#{lesson_paper.entity_id}/pages/#{page['id']}/questions"
  ...
  def sync_answer_result
    answer_detail.each do |key, value|
      ...
      que_detail = SurveyMonkey.get "surveys/#{lesson_paper.entity_id}/pages/#{page_id}/questions/#{que_id}"
  ...
  def set_referenced
    self.class.transaction do
      ...
      Exam.where.not(id: self.id).where(lesson_paper_id: self.lesson_paper_id).update_all(referenced: false)
    end
  end

  def referenced_exam
    @referenced_exam ||= Exam.find_by(lesson_paper_id: self.lesson_paper_id, referenced: true)
  end
